...
#define SCENE_MOUSEDOWN App->Mouse1Click
#define SCENE_MOUSEPRESS App->Mouse1Press
#define SCENE_VEC2 Vec2
#define SCENE_ANIMATION_MODIFIER App->Interval
#define SCENE_SCREENSIZE App->ScreenSize
...
#define SCENE_GL_SCROLL App->Scroll
#define SCENE_COLOR_BLEND_FLOAT(c1,c2,p) c1.BlendFloat(c2, p)
#define SCENE_EDIT_TEXT_ANIM App->AcceptTimeAnimation
#define SCENE_CURTIME App->Curtime

//custom for client
#define SCENE_INIT_TEXTURE(text, data, datalen) if(text == nullptr) text = Render::CreateTexture(data, datalen);
#define SCENE_FTEXT(col,Texture) { Render::DrawTexture(0,0,SCENE_SCREENSIZE.x, SCENE_SCREENSIZE.y,Texture,col); }
#define SCENE_TEXTURE(x, y, w, h, col, Texture) { Render::DrawTexture(x,y,w,h,Texture,col); }
...
if(_State_Item_Hovered && _State_MouseClick ) { /*_State_MouseClick = false;*/ AcceptText = true; SCENE_EDIT_TEXT_ANIM = SCENE_CURTIME; } else if(_State_MouseClick) {AcceptText = false; HoverAnimation = 0.f;}\
if(AcceptText) { ImGui::GetIO().WantTextInput = true;\
	if ((int((SCENE_CURTIME - SCENE_EDIT_TEXT_ANIM) * 0.0019f) % 2) == 0)\
		Written += "|";\
		for (auto c : App->GetInputCharacters()) {\
			if (c == VK_RETURN) {\
				AcceptText = false;\
				onenter   \
				continue;\
...
#define SCENE_CLICKABLE(x,y,w,h, onclick)  _State_Item_Hovered = _SCENE_INREGION(_State_X + x - 3 , _State_Y + y - 3 , w + 3, h+ 3); \
if(_State_Item_Hovered && _State_MouseClick ) { onclick }\
